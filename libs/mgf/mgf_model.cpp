...
bool model::load_file(std::string file){
	...
	for(unsigned int i = 0; i < indices_vertex.size(); i++){
		indices_vertex[i] -= 1;
		m->vertices.push_back(vertices[indices_vertex[i]]);
	}
	if(m->has_texcoords){
		for(unsigned int i = 0; i < indices_texcoords.size(); i++){
			indices_texcoords[i] -= 1;
			m->texcoords.push_back(texcoords[indices_texcoords[i]]);
		}
	}
	if(m->has_normals){
		for(unsigned int i = 0; i < indices_normals.size(); i++){
			indices_normals[i] -= 1;
			m->normals.push_back(normals[indices_normals[i]]);
		}
	}
	...
	for(unsigned int i = 0; i < m->texcoords.size(); i++)
		std::cerr << "texcoords: " << i + 1 << " : " << m->texcoords[i][0] << " " << m->texcoords[i][1] << std::endl;*/
...
void model::render(){
	...
	glDrawArrays(GL_TRIANGLES, 0, meshes[0]->vertices.size() * sizeof(glm::vec3));
	return;
}
