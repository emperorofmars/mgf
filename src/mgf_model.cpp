...
bool model::load_to_buffers(){
	...
	for(unsigned int i = 0; i < meshes.size(); i++){
		...
		if(meshes[i]->has_normals){
			glGenBuffers(1, &meshes[i]->normalbuffer);
			glBindBuffer(GL_ARRAY_BUFFER, meshes[i]->normalbuffer);
			glBufferData(GL_ARRAY_BUFFER, meshes[i]->normals.size() * sizeof(glm::vec3), glm::value_ptr(meshes[i]->normals[0]) /*&meshes[i]->normals[0]*/, GL_STATIC_DRAW);
